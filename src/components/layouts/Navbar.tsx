      ...
      <div className="w-full md:container mx-auto fixed bottom-0 md:bottom-auto md:top-3 xl:top-4 left-[50%] translate-x-[-50%] z-50 flex justify-center">
        ...
        >
            ...
            {ROUTES.map((route, i) => (
              <li key={i}>
                <NavLink
                  ...
                  className={`block px-2 text-white/70 transition hover:text-white/100 ${
                    active === route.link ? "font-semibold text-white/100" : ""
                  }`}
                >
                  {route.label}
                </NavLink>
          ...
          <button className="bg-white font-medium text-primary rounded-full overflow-hidden">
            <NavLink
              to={"/hireme"}
              className="py-2.5 lg:py-3 px-4 lg:px-6 block text-sm"
            >
              Hire me
            </NavLink>
          </button>
        ...
        <div className="w-full px-2 py-3 bg-primary/90 backdrop-blur-sm flex md:hidden items-center justify-between">
            ...
            {ROUTES.map((route, i) => (
              <li key={i} className="w-[60px]">
                <NavLink
                  to={route.link}
                  onClick={() => setActive(route.link)}
                  className={`flex flex-col text-xs items-center gap-2 md:hidden px-2 text-white/70 transition hover:text-white/100 font-light ${
                    active === route.link ? "text-white/100" : ""
                  }`}
                >
                  {active === route.link ? (
          ...
          <button className="absolute right-3 bottom-[110%] bg-primary/90 hover:bg-primary transition backdrop-blur-sm font-medium text-white rounded-full overflow-hidden">
